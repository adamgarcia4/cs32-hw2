...
bool pathExists(char maze[][10], int sr, int sc, int er, int ec)
{
	...
	Coord start(sr, sc);
	mazeQueue.push(start); // push initial coord to stack
	...
	while (!mazeQueue.empty())
	{
		...
		int cr = mazeQueue.front().r();
		int cc = mazeQueue.front().c();
		mazeQueue.pop();
		i++;
